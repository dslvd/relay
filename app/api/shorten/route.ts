...
function isSafeLocalTarget(request: NextRequest, url: string): boolean {
  // Only allow shortening URLs on this same site, and only under /d/ or legacy /download/.
  try {
    const base = request.nextUrl.origin;
    const parsed = new URL(url, base);
    if (parsed.origin !== base) return false;
    return parsed.pathname.startsWith('/d/') || parsed.pathname.startsWith('/download/');
  } catch {
    return false;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isSafeLocalTarget(request, target)) {
      return NextResponse.json({ error: 'Only /d/* (or legacy /download/*) URLs on this site can be shortened' }, { status: 400 });
    }
